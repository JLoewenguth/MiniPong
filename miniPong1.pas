...
procedure terr(i,j:integer);		// terrain
begin
   textcolor(lightgray);		// coins
   gotoxy(1,1);				// possibilite de designs differents pour chacun
   write('O');
   gotoxy(1,21);
   write('O');
   gotoxy(45,1);
   ...
   gotoxy(45,21);
   write('O');
   for i:=1 to 43 do			// bords horizontaux
   begin
       gotoxy(1+i,1);
       write('-');
       gotoxy(1+i,21);
   ...
   for j:=1 to 19 do			// bords verticaux
   begin
       gotoxy(1,1+j);
       write('|');
       gotoxy(45,1+j);
...
BEGIN
   init;
   textcolor(lightred);				//affiche scores 0
 	gotoxy(16,21);
	write(' ');
	write('0');
	write(' ');
   ...
   begin
      ...
      begin
          touche:=readkey;
          if touche=#27 then stop:=true;  //termine la partie
          if touche=#122 then
              mover(-1)
